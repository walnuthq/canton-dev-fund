...
For local Daml Script/test workflows, `dpm trace test <script-or-test> --interactive` uses the same record/replay debugger model. Because the workflow is run locally, the trace bundle can include richer context such as script/test structure, command boundaries, logs, inputs, source metadata, and generated updates.
...
Milestones 2 and 3 deliver interactive trace debugging without requiring Canton node or Daml-LF interpreter changes. Milestone 4 then defines the debug-info format and runtime-hook requirements needed for deeper expression-level stepping.
